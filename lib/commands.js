...
Commands.prototype = {
  LOGIN: function(user, username) {
    ...
    steem.api.getAccounts([username], function(err, results) {
      if (results.length == 1) {
        ...
        user.send(self.server.host, 'CHALLENGE', encodedMessage.substring(1));
      } else {
        console.log('error')
      }
    });
  },
  LOGIN2: function(user, username, uuid) {
    var self = this.server;
    for (let i = tokens.length-1; i >= 0; i--) {
      if (tokens[i].uuid == uuid && tokens[i].username == username) {
        nick = tokens[i].username.trim()
        user.send(user.mask, 'NICK', ':' + nick);
    
        user.channels.forEach(function(channel) {
          //var users = channel.users.splice(channel.users.indexOf(user), 1);
          channel.sendToGroup(channel.users, user.mask + ' NICK : ' + nick);
        });
  ...
  NICK: function(user, nick) {
    ...
    //nick = nick.trim();
    nick = 'ANON'+(Math.floor(Math.random() * 100000) + 1)
    user.send(user.mask, 'NICK', ':' + nick);
  ...
  KICK: function(user, channels, users, kickMessage) {
    ...
    if (userNames.length !== channelMasks.length) {
      ...
      channelMasks.forEach(function(channelMask, i) {
        ...
        var targetUser = channel.findUserNamed(userName);
